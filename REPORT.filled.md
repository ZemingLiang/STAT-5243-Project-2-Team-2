...
> **Note for graders.** If you are looking for the rendered report with all numbers filled in, see [`report.pdf`](report.pdf) (the canonical submission deliverable) or the rendered Markdown at [`REPORT.filled.md`](REPORT.filled.md). This file (`REPORT.md`) is the *template*: numeric values appear as brace-delimited tokens that get substituted by the analysis pipeline (`ab_analysis.py --fill-template REPORT.md`).
...
| Role | Metric (column in `ab_test_events.csv` aggregation) | Definition | Scale | Statistical test |
|------|------|---------|------|------|
| Primary | Apply rate | applies / (previews + applies), per session | [0, 1] continuous | Welch's *t* + Mann-Whitney U |
| Secondary | Preview-to-apply conversion | indicator = 1 if session had ≥ 1 preview AND ≥ 1 apply | binary | Two-proportion *z* |
| Secondary | Apply success rate | apply_success / apply, among sessions with ≥ 1 apply | [0, 1] continuous | Welch's *t* |
| Secondary | Successful actions per session | count of `apply_clean` events with `success = True` | integer ≥ 0 | Mann-Whitney U |

Internal column names (used by `ab_analysis.py`): `apply_rate`, `preview_to_apply_conversion`, `apply_success_rate`, `successful_actions_per_session`.
...
### 3.1 Planned recruitment channels

The single deployment URL was shared in three channels during the collection window:
...
Real public engagement during the 26-hour window was thin (a handful of sessions from team members and a few classmates). The downstream consequence — and the rationale for supplementing with a simulated dataset — is documented in §3.3.1.
