...
Provides:
1. DataFrame-level feature engineering via ``apply_feature_engineering_to_df``.
2. Dataset-store-integrated feature engineering via ``apply_feature_engineering``.
...
Every transformation helper returns a ``(DataFrame, metadata-dict)`` tuple so
the UI layer can display provenance information (formula, columns created, etc.)
without re-inspecting the data.
"""
...
# Generic JSON-friendly response helpers
...
def _validate_columns(df: pd.DataFrame, columns: list[str]) -> None:
    """Raise ValueError if any listed columns are missing from the DataFrame."""
...
def _apply_standardize_feature(
    ...
    *,
    ...
    """Standardize a numeric column: z = (x - mean) / std."""
    _require_numeric(df, [col1])
    ...
    *,
    ...
    try:
        ...
        if method_norm == "log":
            ...
            out, meta = _apply_interaction_feature(df, col1, col2, new_column=new_column)

        elif method_norm == "ratio":
            if not col1 or not col2:
                return _error("col1 and col2 are required for ratio.")
            out, meta = _apply_ratio_feature(df, col1, col2, new_column=new_column)
...
# ---------------------------------------------------------------------------
# Dataset-store-level public API
# ---------------------------------------------------------------------------
    ...
    *,
    ...
    try:
        ...
        if method_norm == "log":
            ...
            if not col1 or not col2:
                return _error("col1 and col2 are required for interaction.")
            df_new, meta = _apply_interaction_feature(df, col1, col2, new_column=new_column)

        elif method_norm == "ratio":
            if not col1 or not col2:
                return _error("col1 and col2 are required for ratio.")
            df_new, meta = _apply_ratio_feature(df, col1, col2, new_column=new_column)
        ...
        return _success(
            {
                ...
                "feature_meta": _json_ready(meta),
                "new_columns": new_cols,
                "dataset_metadata": _json_ready(updated_meta),
                "dataset_summary": summary,
                "preview": preview,
            },
        ...
        return _success(
            {
                ...
                "feature_meta": _json_ready(meta),
                "new_columns": new_cols,
                "dataset_metadata": _json_ready(derived_meta),
                "dataset_summary": summary,
                "preview": preview,
            },
